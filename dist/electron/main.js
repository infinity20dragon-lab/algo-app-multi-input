...
function setupNativeAudioIPC() {
    electron_1.ipcMain.handle('native-audio:is-available', () => {
        return loadNaudiodon();
    });
    electron_1.ipcMain.handle('native-audio:list-devices', () => {
        if (!loadNaudiodon())
            return [];
        try {
    ...
    electron_1.ipcMain.handle('native-audio:start-capture', (_event, config) => {
        if (!loadNaudiodon())
            return false;
    ...
    electron_1.ipcMain.handle('native-audio:stop-capture', (_event, deviceId) => {
        stopCapture(deviceId);
    });
    electron_1.ipcMain.handle('native-audio:stop-all', () => {
        stopAllCaptures();
    });
}
